...
- [NEVER write to the production database](feedback_never_write_production_db.md) — reads fine, every write is the user's to run. 2026-09-02: an "additive" field added to EXPENSE_CATEGORIES took the prod ledger down for 31 min, because the DEPLOYED parser rejects unknown keys. Validate prod config against `git show HEAD`, never the working tree. Deploy code first, then config.
...
- [Property parcel lookup](feature_property_parcel_lookup.md) — free county acreage + overhead imagery on job cards, no API keys. Endpoints are settings; response shapes still hardcoded (deferred). Orange County publishes no addresses — matching degrades there and says so.
- [Forecast tool — Super → Money → Forecast](project_forecast_tool.md) — advisory pay-structure simulator built 2026-09-02; works end to end but the USER HAS NOT USED IT YET. Shared payout math in `packages/money`, cost behavior in its own setting, migration applied to DEV ONLY (prod needs `migrate deploy`). Known gap: changes rates well, can't find leaks.
- [Size findings to their real impact; don't pad explanations](feedback_dont_inflate_findings.md) — user reads length as self-justification. "I don't need a fucking novel." Own mistakes in two sentences; check whether a pushback is right before defending it.
- [Never ship a red spec](feedback_never_ship_a_red_spec.md) — a failing e2e in the final pre-deploy run is a blocker; one was called flaky and took the Ledger tab down in prod 2026-09-08.
- [E2E localStorage stamp race](feedback_e2e_localstorage_stamp_race.md) — waitForLoadState BEFORE stamping topTab, or React's mount clobbers it; day-dependent flake.
- [Detached Prisma include bypasses TypeScript](reference_prisma_detached_include.md) — lift an include/select into a `const` and TS stops key-checking it; use `Prisma.validator<>()`. Gated in job-materials-build-gate.
- [Expenses vs. charges vs. supplies — the two-books model](reference_expenses_charges_supplies.md) — the Ledger is the only deduction; job charges and supplies never write to it, and the ledger link is a breadcrumb no total reads.
