...
| **Payment timing** | **Work-anchored.** Paid every payroll cycle for the work done in that period — even if the client hasn't paid yet. The business fronts the cash. | **Payment-anchored.** Paid AFTER the client pays. If the client never pays, the contractor doesn't get paid (pro-rata on underpayment, $0 on a write-off). |
