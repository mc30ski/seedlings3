...
| Contractor labor (1099) | **Currently:** same `qb-journal-expenses.csv` upload (the app emits Contract Labor rows for both post-GP splits and GP wage-path work). **After connecting Gusto-QB:** Gusto's integration posts contractor payments to QB directly — drop the app's Contract Labor rows on import (or remove the Contract Labor mapping from EXPENSE_CATEGORIES so they land as Unmapped for manual review). | Contract Labor expense |
