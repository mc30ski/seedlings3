...
export default function ServicesTab({
  ...
  const tabHelp = (
        <TabExplainer
          explainerId={`seedlings:servicesTab:guideOpen:${isSuper ? "super" : "admin"}`}
        >
          <ExplainerText>
            The recurring services themselves — the job behind the visits. A service carries
            the price, the cadence and the <Em>default team</Em> that gets put on each new
            visit. Create and edit them here, add a visit by hand, and archive a service that
            has run its course.
          </ExplainerText>
          <ExplainerText>
          ...
          {isSuper && (
            <RoleSection role="Super">
              <ExplainerText>
                You get that Delete on an archived service or visit — and a service will not
                delete while it still has visits attached.
              </ExplainerText>
            </RoleSection>
          )}
        </TabExplainer>
                            ...
                            {(occ.estimatedMinutes != null || (occ.startedAt && occ.completedAt)) && (
                              <HStack fontSize="xs" gap={2}>
                                {occ.estimatedMinutes != null && (() => {
                                  ...
                                  const adj = workerCount > 1 ? Math.round(occ.estimatedMinutes / workerCount) : occ.estimatedMinutes;
                                  return <Text color="fg.muted">Est: {adj >= 60 ? `${Math.floor(adj / 60)}h ${adj % 60}m` : `${adj}m`}{workerCount > 1 ? ` (${workerCount} workers)` : ""}</Text>;
                                })()}
                                {occ.startedAt && occ.completedAt && (() => {
                                  const workerCount = Math.max(1, (occ.assignees ?? []).filter((a: any) => a.role !== "observer").length);
                                  const actual = (new Date(occ.completedAt).getTime() - new Date(occ.startedAt).getTime() - ((occ as any).totalPausedMs ?? 0)) / 60000;
