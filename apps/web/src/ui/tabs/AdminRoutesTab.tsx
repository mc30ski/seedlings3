...
export default function AdminRoutesTab() {
  ...
  const [selectedWorker, setSelectedWorker] = usePersistedState<string | null>("adminroutes_worker", null);
  ...
  return (
    <Box w="full">
      ...
      {selectedWorker ? (
        <PreviewRoutesTab key={selectedWorker} userId={selectedWorker} />
      ) : (
        <Box textAlign="center" py={10}>
          <Text color="fg.muted">Select a worker to analyze their routes.</Text>
        </Box>
      )}
    </Box>
  );
